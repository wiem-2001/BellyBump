...
public function show(int $id, RendezVousRepository $rendezVousRepository): Response
{
    ...
    // Récupérer le rendez-vous à afficher en utilisant l'identifiant de la route
    $rendezVou = $rendezVousRepository->find($id);

    // Vérifier si le rendez-vous existe
    if (!$rendezVou) {
        throw $this->createNotFoundException('RendezVous non trouvé avec l\'identifiant : '.$id);
    }
    ...
    public function edit(Request $request, int $id, RendezVousRepository $rendezVousRepository, EntityManagerInterface $entityManager): Response
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');
    
        // Récupérer le rendez-vous à éditer en utilisant l'identifiant de la route
        $rendezVou = $rendezVousRepository->find($id);
    ...
    public function delete(Request $request, int $id, RendezVousRepository $rendezVousRepository, EntityManagerInterface $entityManager): Response
    {
        $this->denyAccessUnlessGranted('ROLE_ADMIN');
    
        // Récupérer le rendez-vous à supprimer en utilisant l'identifiant de la route
        $rendezVou = $rendezVousRepository->find($id);
